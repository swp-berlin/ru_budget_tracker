...
This needs a `src/.env` file with `IMPORTER__NEXTCLOUD_DOWNLOAD_LINK` and
`IMPORTER__DEEPL_API_KEY` (see [`importer.md`](importer.md)); the same target runs inside
the importer container (`Dockerfile.importer`) in deployment.
