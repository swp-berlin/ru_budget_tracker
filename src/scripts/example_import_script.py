...
def load_data_from_file(file_path: Path, budget_db_id: int | None = None) -> dict:
    """
    ...
    try:
        with get_sync_session() as session:
            ...
            existing_budget = (
                ...
                .filter_by(original_identifier=budget.original_identifier)
                .first()
            )
            ...
            logger.info(
                f"Successfully imported budget: {budget.original_identifier} (ID: {budget_db_id})"
            )
            return budget_db_id
    ...
    try:
        with get_sync_session() as session:
            ...
            for dimension in dimensions:
                ...
                logger.debug(
                    f"Created dimension: {dimension.original_identifier} ({dimension.type})"
                )
            ...
            for dimension in dimensions:
                if dimension.parent_id:
                    ...
                    if not db_dimension:
                        logger.warning(
                            f"Could not find DB object for {dimension.original_identifier} ({dimension.type})"
                        )
                        continue
                    ...
                    if parent_db_obj:
                        ...
                        logger.debug(
                            f"Set parent for {dimension.original_identifier} ({dimension.type}) -> {dimension.parent_id} (type: {expected_parent_type})"
                        )
                    else:
                        logger.warning(
                            f"Parent {dimension.parent_id} (expected type: {expected_parent_type}) not found for {dimension.original_identifier} ({dimension.type})"
                        )
            ...
            for expense in expenses:
                ...
                for dim in expense.dimensions:
                    ...
                    if db_dim:
                        ...
                        logger.warning(
                            f"Could not find DB dimension for {dim.original_identifier} ({dim.type})"
                        )
            ...
            session.commit()
            logger.info(
                f"Successfully imported {len(dimensions)} dimensions and {len(expenses)} expenses for budget ID {budget_db_id}"
            )
