...
from models import Dimension, Expense, DimensionTypeLiteral
...
def find_header_row(df: pd.DataFrame) -> int:
    ...
    for idx, *row in df.itertuples():
        ...
        if "Наименование" in row_str and ("Мін" in row_str or "Мин" in row_str):
            return idx  # Return the index of the header row
...
def merge_rows(
    df: pd.DataFrame, header_row_idx: int, col_mapping: Dict[str, int]
) -> list[MergedRow]:
    """
    ...
    for idx in range(header_row_idx + 1, len(df)):
        ...
        if not has_codes:
            ...
            if merged_rows:
                ...
                prev_ends_with_paren: bool = prev_entry.name.endswith(")")
                ...
                if prev_has_expense_type and not prev_ends_with_paren:
                    ...
                    logger.debug(
                        f"Row {idx}: Appended to previous row (has expense_type, no closing paren)"
                    )
                    continue
                ...
                if not prev_has_expense_type and current_ends_with_quote:
                    prev_entry.name += " " + name
                    logger.debug(
                        f"Row {idx}: Appended to previous row (no expense_type, ends with quote)"
                    )
                    continue
...
def parse_dimensions_from_merged_rows(merged_rows: List[MergedRow]) -> List[Dimension]:
    """
    ...
    for entry in merged_rows:
        ...
        if expense_type_code:
            ...
            logger.debug(
                f"Processing expense type {expense_type_code}: extracted name '{expense_type_name[:100]}'"
            )
...
def deduplicate_dimensions(dimensions_list: List[Dimension]) -> List[Dimension]:
    ...
    identifier_type_names: dict[tuple[str, DimensionTypeLiteral], list[str]] = {}

    for dim in dimensions_list:
        key_1 = (dim.original_identifier, dim.type)
        if key_1 not in identifier_type_names:
            identifier_type_names[key_1] = []
        identifier_type_names[key_1].append(dim.name)
    ...
    for (identifier, dim_type), names in identifier_type_names.items():
        ...
        if len(unique_names) > 1:
            logger.warning(
                f"Same {dim_type} '{identifier}' has {len(unique_names)} different names:"
            )
    ...
    seen = set()
    unique_dimensions: list[Dimension] = []
    duplicates_removed = 0

    for dim in dimensions_list:
        key_2: tuple[str, DimensionTypeLiteral, str] = (dim.original_identifier, dim.type, dim.name)
    ...
    logger.info(
        f"Removed {duplicates_removed} exact duplicates, "
        f"{len(unique_dimensions)} unique dimensions remain"
    )

    # Count by type
    type_counts: dict[DimensionTypeLiteral, int] = {}
...
def create_expenses_from_merged_rows(
    merged_rows: List[MergedRow], dimensions_list: List[Dimension], budget_id: int
) -> List[Expense]:
    """
    ...
    for row in merged_rows:
        ...
        expense = Expense(budget_id=budget_id, value=row.value)
        ...
        if expense.dimensions:
            ...
            logger.debug(
                f"Created expense with value {row.value}, linked to: {', '.join(linked_dims)}"
            )
        else:
...
def extract_dimensions_and_expenses_from_excel(
    excel_file_path: Path, budget_id: int
...
def load_dimensions_and_expenses_from_excel(
    excel_file_path: Path, budget_id: int | None = None
) -> dict:
    """
    ...
    if budget_id is None:
        ...
        return {"dimensions": dimensions, "expenses": []}
    ...
    return {"dimensions": dimensions, "expenses": expenses}
