...
def process_laws(raw_dir, clean_dir):
    ...
    for xlsx_file in raw_path.glob("*.xlsx"):
        ...
        try:
            ...
            logger.info(f"✓ {xlsx_file.name} → law_{year}.xlsx")
        except Exception as e:
            logger.error(f"✗ {xlsx_file.name}: {e}")
...
def process_reports(raw_dir, clean_dir):
    ...
    for file in list(raw_path.glob("*.xls")) + list(raw_path.glob("*.xlsx")):
        ...
        try:
            ...
            logger.info(f"✓ {file.name} → {output_filename}")
        except Exception as e:
            logger.error(f"✗ {file.name}: {e}")


def main():
    raw_dir = settings.importer.raw_dir
    data_dir = settings.importer.data_dir
