...
## ExpenseDimensions
The `ExpenseDimensions` model is an association table that links expenses to dimensions. It contains two primary key fields: `expense_id` and `category_id`, which are foreign keys referencing the `Expense` and `Dimension` models, respectively. This table allows for a many-to-many relationship between expenses and dimensions, enabling each expense to be associated with multiple dimensions and vice versa.
